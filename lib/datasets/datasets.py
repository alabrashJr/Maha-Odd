...
def get_loader(dataset_name, **kwargs):
    ...
    if dataset_name == "rostd":
        ...
        loader = partial_class(CLINC150Loader,
                               data_path=kwargs.get("data_path","data/clinc/data_full.json"),
                               unsupervised=True)
...
def get_test_transformers(tokenizer, dataset_name,**kwargs):
    loader = get_loader(dataset_name, **kwargs)
    dataset = OODDataset(loader(subset='train'), tokenizer.tokenize)
    dataset.vectorize_texts(tokenizer)
    return dataset
